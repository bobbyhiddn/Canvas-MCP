...
Example YAML recipes demonstrating the Canvas-MCP hierarchical diagram format.
...
Use the `render_canvas` MCP tool or render directly:

```bash
uv run python -c "
from canvas_mcp.parser import parse_yaml
from canvas_mcp.renderer import render_canvas

with open('examples/rhode-architecture.yaml') as f:
    canvas = parse_yaml(f.read())
render_canvas(canvas, scale=2.0, filename='rhode-architecture')
"
```

Or with the organize algorithm for auto-layout:
...
with open('examples/risk-chain.yaml') as f:
    canvas = parse_yaml(f.read())
organize_canvas(canvas)
render_canvas(canvas, scale=2.0, filename='risk-chain')
"
```

## Notes

- Some examples use extended node types (`pdf_file_source`, `text_file_source`, `text_file_output`) that Canvas-MCP will render as `default` type nodes.
